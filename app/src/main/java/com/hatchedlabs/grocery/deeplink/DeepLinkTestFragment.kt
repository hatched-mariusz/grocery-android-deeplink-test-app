...
class DeepLinkTestFragment : Fragment(),
    ...
    private fun loadExamples(): List<LinkExample> {
        return listOf(
            LinkExample(
                "Run Grocery app by http",
                "https://${getCurrentHost()}/shop"
            ),
            LinkExample(
                "Run Grocery installed app ",
                "${getCurrentSchema()}://${getCurrentHost()}/main/search"
            ),
            LinkExample(
                "Go to search screen",
                "${getCurrentSchema()}://${getCurrentHost()}/main/search?query=pizza"
            ),
            LinkExample(
                "search in category screen",
                "${getCurrentSchema()}://${getCurrentHost()}/main/search?category=Prepared%20Foods&query=three%20cheese%20pizza"
            ),
//         TODO   LinkExample("Go to search screen with selected store", "${SCHEMA}://${HOST}/main/search?selectedStore=72&fulfillmentMethod=pickup"),
            ...
            LinkExample(
                "Change to store",
                "${getCurrentSchema()}://${getCurrentHost()}/home/store?slug=westerville-healthcare"
            ),
            LinkExample(
                "Show user cart",
                "${getCurrentSchema()}://${getCurrentHost()}/main/cart"
            ),
            LinkExample(
                "Show user order confirmation",
                "${getCurrentSchema()}://${getCurrentHost()}/confirm/order?id=699KF66D"
            ),
            LinkExample(
                "Show order history",
                "${getCurrentSchema()}://${getCurrentHost()}/orders"
            ),
            LinkExample(
                "Show user past order",
                "${getCurrentSchema()}://${getCurrentHost()}/order/detail?id=699KF66D"
            ),
            LinkExample(
                "Show user quick list",
                "${getCurrentSchema()}://${getCurrentHost()}/main/quicklist"
            ),
            LinkExample(
                "Show curated list",
                "${getCurrentSchema()}://${getCurrentHost()}/shop/curatedList?id=2"
            ),
            LinkExample(
                "Show user account",
                "${getCurrentSchema()}://${getCurrentHost()}/main/account"
            ),
            LinkExample(
                "Change user phone number",
                "${getCurrentSchema()}://${getCurrentHost()}/enterphone"
            ),
            LinkExample(
                "Change user time slot",
                "${getCurrentSchema()}://${getCurrentHost()}/timeselector"
            ),
            LinkExample(
                "Run barcode product scanner",
                "${getCurrentSchema()}://${getCurrentHost()}/barcode/product"
            ),
            LinkExample(
                "Run barcode card scanner",
                "${getCurrentSchema()}://${getCurrentHost()}/barcode/card"
            ),
            LinkExample(
                "create new GEAC card",
                "${getCurrentSchema()}://${getCurrentHost()}/account/card/signup"
            ),
            LinkExample(
                "link GEAC card",
                "${getCurrentSchema()}://${getCurrentHost()}/account/card/add"
            ),
            LinkExample(
                "ads GEAC card",
                "${getCurrentSchema()}://${getCurrentHost()}/account/card/advertisement"
            ),
            LinkExample(
                "Run barcode card scanner",
                "${getCurrentSchema()}://${getCurrentHost()}/barcode/card"
            ...
            LinkExample(
                "Show OSS licenses",
                "${getCurrentSchema()}://${getCurrentHost()}/licenses/all"
            ),
            LinkExample(
                "Braze Inbox",
                "${getCurrentSchema()}://${getCurrentHost()}/inbox"
            ),
            LinkExample(
                "Store not in store chooser",
                "${getCurrentSchema()}://${getCurrentHost()}/home/store?slug=tionesta"
            )
